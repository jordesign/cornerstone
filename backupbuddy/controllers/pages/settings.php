...
if ( pb_backupbuddy::_POST( 'pb_backupbuddy_backup_directory' ) != '' ) {
	...
	if ( pb_backupbuddy::$options['backup_directory'] != $backup_directory ) { // Directory differs. Needs updated in post var. Give messages here as this value is going to end up being saved.
		...
		pb_backupbuddy::alert( 'Your backup storage directory has been updated from "' . $old_backup_dir . '" to "' . $new_backup_dir . '". ' . $old_backups_moved . ' backup(s) have been moved to the new location. You should perform a manual backup to verify that your backup storage directory changes perform as expected. Backups in your previous directory have not been moved. You should delete, move, or exclude their directory from your backups.' );
	}
}
...
$settings_form->add_setting( array(
	...
	'title'		=>		__('Storage directory for backups', 'it-l10n-backupbuddy' ) . ' (BETA)',
	'tip'		=>		__('Directory where all backup ZIP files will be saved to. This directory must have proper write and read permissions. Upon changing, any backups in the existing directory will be moved to the new directory.', 'it-l10n-backupbuddy' ),
	'rules'		=>		'required',
	'css'		=>		'width: 325px;',
